# -*- coding: utf-8 -*-
"""Copy of hw7_Weight_Quantization.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1ZNtS6-rHJiOUhX_CJ6QSj5PtOWY0vy7R
...
print(f"\noriginal cost: {os.stat('/content/student_model (8).bin').st_size} bytes.")
params = torch.load('/content/student_model (8).bin')
...
print(f"8-bit cost: {os.stat('test.bin').st_size} bytes.")

import lzma

def convert(number_array):
    temp = np.unpackbits(number_array)
    bit_array = temp.reshape(-1,8)
...
def encode8(params, fname):
    ...
    for (name, param) in params.items():
        ...
        if type(param) == np.ndarray:
            ...
            max_val = np.max(param)
            shape = param.shape
            param = np.round((param - min_val) / (max_val - min_val) * 63)
            param = np.uint8(param)
            param =  convert(param)
    ...
    pickle.dump(custom_dict, open(fname, 'wb'))
    with open("test.bin", 'wb') as fp:
        fp.write(zlib.compress(pickle.dumps(custom_dict,4)))
...
def decode8(fname):
    ...
    for (name, param) in params.items():
        if type(param) == tuple:
            ...
            param = np.float64(param)
            param = (param / 63 * (max_val - min_val)) + min_val
            param = torch.tensor(param)
        else:
            param = torch.tensor(param)
        custom_dict[name] = param
...
encode8(params, '8_bit_model.pkl')
print(f"8-bit cost: {os.stat('8_bit_model.pkl').st_size} bytes.")
print(f"test.bin: {os.stat('test.bin').st_size} bytes.")
newdict = decode8("test.bin")
...
def encode8(params, fname):
    ...
    for (name, param) in params.items():
        ...
        if type(param) == np.ndarray:
            ...
            custom_dict[name] = param.astype(np.float16)
    pickle.dump(custom_dict, open(fname, 'wb'))
    with open("test.bin", 'wb') as fp:
        fp.write(lzma.compress(pickle.dumps(custom_dict,4),preset = 9 ,format = 2 ))
...
tt = np.uint16(((127,5),(33,44)))

#ttt = convert(tt,)
#print(ttt)
#print(inconvert(ttt,tt.shape))
c =  pickle.dumps(True)
for i in range(2**10):
    c += pickle.dumps(True)
pickle.dump(c,open("fdde", 'wb'))
